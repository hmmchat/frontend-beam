...
function VideoChatContent() {
    ...
    const init = async () => {
      ...
      if (token) {
        try {
          ...
          setLocalUserInfo(initialInfo);
          localUserInfoRef.current = initialInfo;

          // Fetch full user profile to get real displayPictureUrl
          apiRequest(API.USERS.GET_ME)
            .then((meData) => {
              const meUser = meData?.user || meData;
              if (meUser) {
                const info = {
                  name: meUser.username || payload.name || 'You',
                  age: payload.age || '',
                  displayPictureUrl: meUser.displayPictureUrl || '/assets/ico.png'
                };
                localUserInfoRef.current = info;
                setLocalUserInfo(info);
              }
            })
            .catch((err) => {
              console.warn('[Init] Failed to fetch my profile info:', err);
            });
        } catch { }
      }
  ...
  const triggerReportToast = (message) => {
    setReportNotification(message);
    setTimeout(() => {
  ...
  const handleReportUser = async (reportedUserId) => {
    ...
    if (!tid || tid.startsWith('producer:')) {
      triggerReportToast("Cannot report this user type.");
      return;
    }
    if (reportedUserIds.has(tid)) {
      triggerReportToast("You have already reported this user.");
      return;
    }
    ...
    try {
      ...
      if (res.success) {
        ...
        triggerReportToast("User reported successfully.");
      } else {
        triggerReportToast("Failed to report user.");
      }
    } catch (err) {
      console.error("[Report] failed:", err);
      triggerReportToast(err.message || "Failed to report user.");
    }
  };
  ...
  const handleCancelPullStranger = async () => {
    ...
    try {
      ...
      await apiRequest(API.STREAMING.DISABLE_PULL_STRANGER(roomInfo.roomId), {
        method: 'POST',
        body: JSON.stringify({ userId: userIdRef.current })
      }).catch(() => { });
