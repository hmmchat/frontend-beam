...
async function fetchJson(url, options = {}) {
  ...
  } catch (e) { throw new Error(fetchNetworkMessage(e)); }
  if (!res.ok) {
    ...
    if (res.status === 429) throw new Error(`${msg} — please wait a moment and try again.`);
    throw new Error(msg);
  }
  const ct = res.headers.get("content-type");
...
export default function Inbox() {
  ...
  const enrichUser = useCallback(async (userId) => {
    try {
      ...
      return { id: userId, username: "User", displayPictureUrl: null, preferredCity: "" };
    }
  }, []);
  ...
  const loadGiftCatalog = useCallback(async () => {
    ...
    try {
      ...
      if (typeof data?.firstMessageCostCoins === "number") setFirstMessageCost(data.firstMessageCostCoins);
      const mapped = mapCatalogToModalGifts(data?.gifts || []);
      setGiftModalItems(mapped.length ? mapped : null);
    } catch { setGiftModalItems(null); }
    finally { setGiftsCatalogLoading(false); }
  }, []);

  useEffect(() => { refreshWallet(); loadGiftCatalog(); }, [refreshWallet, loadGiftCatalog]);
  ...
  const scheduleNotificationBadge = useCallback(() => {
    ...
    const now = Date.now();
    if (now >= notifNextAllowedAtRef.current) { void fire(); return; }
  ...
  useEffect(() => () => {
    if (notifBadgeTimerRef.current) { clearTimeout(notifBadgeTimerRef.current); notifBadgeTimerRef.current = null; }
  }, []);

  const loadLists = useCallback(async (opts = {}) => {
    const quiet = opts.quiet === true;
    const skipNotificationBadge = opts.skipNotificationBadge === true;
    const token = localStorage.getItem("accessToken");
    if (!token) { router.push("/"); return; }
    const uid = resolveUserIdFromToken();
    if (uid) { setCurrentUserId(uid); localStorage.setItem("userId", uid); }
    ...
    try {
      ...
      if (activeTab === "inbox") {
        const data = await fetchJson(API.FRIENDS.getInboxConversationsUrl({ limit: LIST_LIMIT, filter: filterParam }));
        ...
        const recvData = await fetchJson(API.FRIENDS.getReceivedRequestsUrl({ limit: LIST_LIMIT, filter: filterParam }));
        ...
        if (filterParam !== MsgFilter.TEXT_ONLY && filterParam !== MsgFilter.WITH_GIFT) {
          ...
          const pendingRows = await Promise.all(
            pendingArr.filter((req) => !convOthers.has(String(req.fromUserId))).map(async (req) => {
              const u = await enrichUser(req.fromUserId);
              return {
                id: `pending_fr_${req.id}`, conversationId: `pending_fr_${req.id}`,
                otherUserId: req.fromUserId,
                otherUser: { id: req.fromUserId, username: u.username, displayPictureUrl: u.displayPictureUrl },
                lastMessage: null, unreadCount: 0, isFriend: false,
                isFollowRequest: true, followRequestId: req.id,
                lastMessageAt: req.createdAt, createdAt: req.createdAt,
              };
            })
          );
        ...
        const sentConvData = await fetchJson(API.FRIENDS.getSentRequestsUrl({ limit: LIST_LIMIT, filter: filterParam }));
        ...
        if (filterParam !== MsgFilter.TEXT_ONLY && filterParam !== MsgFilter.WITH_GIFT) {
          ...
          const extraRows = await Promise.all(
            sentArr.filter((r) => !convRecipients.has(String(r.toUserId))).map(async (r) => {
              const u = await enrichUser(r.toUserId);
              return {
                id: `outgoing_fr_${r.id}`, conversationId: `outgoing_fr_${r.id}`,
                otherUserId: r.toUserId,
                otherUser: { id: r.toUserId, username: u.username, displayPictureUrl: u.displayPictureUrl },
                lastMessage: null, unreadCount: 0, isFriend: false,
                isOutgoingFriendRequest: true, outgoingFriendRequestId: r.id,
                lastMessageAt: r.createdAt, createdAt: r.createdAt,
              };
            })
          );
      ...
      if (activeTab === "inbox") { setInboxList([]); setInboxHasMore(false); }
      if (activeTab === "requests") { setRequestsList([]); setRequestsHasMore(false); }
      if (activeTab === "sent") { setSentList([]); setSentHasMore(false); }
    } finally {
  ...
  useEffect(() => {
    setInboxCursor(undefined); setRequestsCursor(undefined); setSentCursor(undefined);
    loadLists();
  }, [loadLists]);

  useEffect(() => { setConversationSearch(""); }, [activeTab]);

  useEffect(() => {
    if (!threadMenuOpen) return;
    const close = (e) => {
      if (threadMenuRef.current && !threadMenuRef.current.contains(e.target)) setThreadMenuOpen(false);
    };
    document.addEventListener("mousedown", close);
    return () => document.removeEventListener("mousedown", close);
  }, [threadMenuOpen]);

  useEffect(() => { setThreadMenuOpen(false); }, [activeChat?.rowKey]);

  useEffect(() => {
    const run = async () => {
      try {
        if (activeTab === "inbox") {
          await apiRequest(API.FRIENDS.MARK_NOTIFICATIONS_SEEN, { method: "POST", body: JSON.stringify({ section: "INBOX" }) });
        } else if (activeTab === "requests") {
          await apiRequest(API.FRIENDS.MARK_NOTIFICATIONS_SEEN, { method: "POST", body: JSON.stringify({ section: "RECEIVED_REQUESTS" }) });
          await apiRequest(API.FRIENDS.MARK_NOTIFICATIONS_SEEN, { method: "POST", body: JSON.stringify({ section: "FRIEND_REQUESTS" }) });
        } else if (activeTab === "sent") {
          await apiRequest(API.FRIENDS.MARK_NOTIFICATIONS_SEEN, { method: "POST", body: JSON.stringify({ section: "SENT_REQUESTS" }) });
        }
  ...
  const loadMore = useCallback(async () => {
    ...
    if (activeTab === "inbox" && inboxHasMore && inboxCursor) {
      setLoadingMore(true);
      try {
        const data = await fetchJson(API.FRIENDS.getInboxConversationsUrl({ limit: LIST_LIMIT, cursor: inboxCursor, filter: filterParam }));
        const next = (data?.conversations || []).filter(shouldShowInboxConversation);
        setInboxList((prev) => sortByLatest(dedupeAppend(prev, next)));
        setInboxCursor(data?.nextCursor); setInboxHasMore(Boolean(data?.hasMore));
    ...
    if (activeTab === "requests" && requestsHasMore && requestsCursor) {
      setLoadingMore(true);
      try {
        const recvData = await fetchJson(API.FRIENDS.getReceivedRequestsUrl({ limit: LIST_LIMIT, cursor: requestsCursor, filter: filterParam }));
        ...
        setRequestsCursor(recvData?.nextCursor); setRequestsHasMore(Boolean(recvData?.hasMore));
      } catch (e) { console.error(e); } finally { setLoadingMore(false); }
      return;
    }
    if (activeTab === "sent" && sentHasMore && sentCursor) {
      setLoadingMore(true);
      try {
        const sentConvData = await fetchJson(API.FRIENDS.getSentRequestsUrl({ limit: LIST_LIMIT, cursor: sentCursor, filter: filterParam }));
        ...
        setSentCursor(sentConvData?.nextCursor); setSentHasMore(Boolean(sentConvData?.hasMore));
      } catch (e) { console.error(e); } finally { setLoadingMore(false); }
    }
  }, [activeTab, msgFilter, inboxHasMore, inboxCursor, requestsHasMore, requestsCursor, sentHasMore, sentCursor, loadingMore]);

  const markReadForPeer = useCallback(async (otherUserId) => {
    if (!otherUserId) return;
    try {
      const res = await fetch(API.FRIENDS.MARK_MESSAGES_READ(otherUserId), { method: "POST", headers: getAuthHeaders() });
  ...
  const computeSidebarUnread = useCallback((conversation, convKey, selected) => {
    ...
    const lastSeenMs = clientLastSeenAtByConvRef.current[convKey] ?? stored?.lastSeenAtMs ?? null;
    if (lastSeenMs == null && !stored) return Math.max(0, Math.floor(Number(conversation.unreadCount || 0)));
    ...
    if (stored?.lastSeenMessageId != null && lm.id != null && String(lm.id) === String(stored.lastSeenMessageId)) return 0;
    const lmT = new Date(lm.createdAt || 0).getTime();
    if (Number.isNaN(lmT)) return Math.max(0, Math.floor(Number(conversation.unreadCount || 0)));
  ...
  const loadThreadMessages = useCallback(async (chat) => {
    ...
    const reqId =
      ...
      (String(chat.conversationId || "").startsWith("pending_fr_") ? String(chat.conversationId).replace("pending_fr_", "") : null) ||
      (String(chat.conversationId || "").startsWith("outgoing_fr_") ? String(chat.conversationId).replace("outgoing_fr_", "") : null);
    setThreadNextCursor(undefined); setThreadHasMore(false);
    try {
      if (!isSyntheticConversationId(cid)) {
        const data = await apiRequest(API.FRIENDS.GET_CONVERSATION_MESSAGES(cid, { limit: THREAD_MSG_LIMIT }));
        ...
        if (msgs?.length) {
          ...
          writeStoredLastSeen(cidStr, { lastSeenAtMs: clientLastSeenAtByConvRef.current[cidStr], lastSeenMessageId: best?.id ?? null });
        } else {
          const now = Date.now();
      ...
      if (reqId && (chat.isFollowRequest || chat.isOutgoingFriendRequest)) {
        ...
        setMessages(arr);
        if (!chat.isOutgoingFriendRequest) await markReadForPeer(chat.otherUser?.id || chat.otherUserId);
        return;
      }
      setMessages([]);
    } catch (e) {
  ...
  const loadOlderThreadMessages = useCallback(async () => {
    ...
    if (root) {
      pendingThreadScrollRestoreRef.current = { scrollHeight: root.scrollHeight, scrollTop: root.scrollTop };
    } else { pendingThreadScrollRestoreRef.current = null; }
    skipScrollToBottomRef.current = true;
    setLoadingThreadOlder(true);
    try {
      const data = await apiRequest(API.FRIENDS.GET_CONVERSATION_MESSAGES(cid, { limit: THREAD_MSG_LIMIT, cursor: threadNextCursor }));
      ...
      setThreadNextCursor(data?.nextCursor); setThreadHasMore(Boolean(data?.hasMore));
    } catch (e) {
      console.error("[Inbox] loadOlderThreadMessages", e);
      pendingThreadScrollRestoreRef.current = null;
    } finally { setLoadingThreadOlder(false); }
  }, [activeChat, loadingThreadOlder, threadNextCursor, threadHasMore]);

  useEffect(() => { if (activeChat) loadThreadMessages(activeChat); }, [activeChat, loadThreadMessages]);
  ...
  const connectFriendsWs = useCallback(() => {
    ...
    wsHeartbeatRef.current = setInterval(() => {
      try { if (ws.readyState === 1) ws.send(JSON.stringify({ type: "ping", data: { at: Date.now() } })); } catch { }
    }, 20000);
    ...
    ws.onmessage = (e) => {
      ...
      if (!msg?.type) return;
      if (msg.type === "ws:ready") { if (msg.data?.ok) wsReconnectAttemptRef.current = 0; return; }
      ...
      if (msg.type === "friend:message" && msg.data) {
        ...
        const lastSeenAtMs = clientLastSeenAtByConvRef.current[convKey] ?? readStoredLastSeen(convKey)?.lastSeenAtMs ?? null;
        if (isIncoming && !isActiveIncoming && convKey && !isSyntheticConversationId(convKey)) {
          if (lastSeenAtMs != null) {
            setClientUnreadByConv((prev) => ({ ...prev, [convKey]: (prev[convKey] ?? 0) + 1 }));
          }
        }
        setMessages((prev) => {
          ...
          if (!m.id) {
            ...
            const has = prev.some((x) => `${x.fromUserId}:${x.toUserId}:${x.createdAt}:${x.messageType}:${x.message || ""}` === key);
            if (has) return prev;
          }
        ...
        const hasServerUnread = rawUnread !== undefined && rawUnread !== null && !Number.isNaN(Number(rawUnread));
        ...
        if (!hasServerUnread && (inInbox || inReq || inSent)) {
          ...
          wsListSyncTimerRef.current = setTimeout(() => { wsListSyncTimerRef.current = null; loadListsFn?.({ quiet: true }); }, 350);
        }
        ...
        if (activeCid2 && String(activeCid2) === String(m.conversationId) && isIncoming) {
          const clearForConv = (setter) => setter((prev) => prev.map((c) => String(c.conversationId || c.id) === String(m.conversationId) ? { ...c, unreadCount: 0 } : c));
          clearForConv(setInboxList); clearForConv(setRequestsList); clearForConv(setSentList);
          void markReadFn?.(ac2?.otherUserId || ac2?.otherUser?.id);
        }
      }

      if (msg.type === "friend:refresh") { loadListsRef.current?.({ quiet: true, skipNotificationBadge: true }); loadNotificationBadgeRef.current?.(); }
      ...
      if (msg.type === "friend:read" && msg.data) {
        ...
        if (!other) return;
        const clearForOther = (setter) => setter((prev) => prev.map((c) => String(c.otherUserId) === String(other) ? { ...c, unreadCount: 0 } : c));
        clearForOther(setInboxList); clearForOther(setRequestsList); clearForOther(setSentList);
      }
    };
  ...
  useEffect(() => { connectFriendsWsRef.current = connectFriendsWs; }, [connectFriendsWs]);

  useEffect(() => {
    connectFriendsWs();
    return () => {
      if (wsReconnectTimerRef.current) { clearTimeout(wsReconnectTimerRef.current); wsReconnectTimerRef.current = null; }
      if (wsListSyncTimerRef.current) { clearTimeout(wsListSyncTimerRef.current); wsListSyncTimerRef.current = null; }
      if (wsHeartbeatRef.current) { clearInterval(wsHeartbeatRef.current); wsHeartbeatRef.current = null; }
  ...
  const emitTyping = useCallback((isTyping) => {
    ...
    try {
      ws.send(JSON.stringify({ type: "friend:typing", data: { conversationId: activeChat.conversationId, otherUserId: activeChat.otherUserId, isTyping: Boolean(isTyping) } }));
    } catch { }
  ...
  useEffect(() => {
    ...
    return () => { if (threadPollRef.current) { clearInterval(threadPollRef.current); threadPollRef.current = null; } };
  ...
  useEffect(() => {
    ...
    enrichUser(oid).then((u) => {
      if (cancelled) return;
      setActiveChat((prev) => prev && prev.rowKey === rk ? { ...prev, otherUser: { ...prev.otherUser, ...u } } : prev);
    });
    return () => { cancelled = true; };
  ...
  useLayoutEffect(() => {
    ...
    sync();
    const outer = requestAnimationFrame(() => { sync(); requestAnimationFrame(sync); });
    return () => cancelAnimationFrame(outer);
  }, [messages]);

  useEffect(() => {
    if (skipScrollToBottomRef.current) { skipScrollToBottomRef.current = false; return; }
    const root = messagesScrollRef.current;
  ...
  const handleAcceptRequest = async (requestId) => {
    ...
    try {
      ...
      setActiveChat(null); await loadLists();
    } catch (e) { console.error(e); alert(e.message || "Failed to accept"); }
  };
  ...
  const handleRejectRequest = async (requestId) => {
    ...
    try {
      ...
      setActiveChat(null); await loadLists();
    } catch (e) { console.error(e); alert(e.message || "Failed to reject"); }
  };
  ...
  const sendOfflineFriendRequest = async (toUserId) => {
    ...
    try {
      await apiRequest(API.FRIENDS.SEND_FRIEND_REQUEST, { method: "POST", body: JSON.stringify({ toUserId }) });
      await loadLists();
    } catch (e) { alert(e.message || "Could not send friend request"); }
    finally { setSendFriendBusy(false); }
  };
  ...
  const sendMessage = async (giftData = null) => {
    ...
    try {
      ...
      if (!isSyntheticConversationId(realCid)) {
        const res = await apiRequest(API.FRIENDS.SEND_MESSAGE(realCid), { method: "POST", body: JSON.stringify(body) });
        if (res?.promotedToInbox) setActiveTab("inbox");
        if (typeof res?.newBalance === "number") setWalletCoins(res.newBalance); else refreshWallet();
      } else if (activeChat.outgoingFriendRequestId) {
        const res = await apiRequest(API.FRIENDS.SEND_REQUEST_MESSAGE(activeChat.outgoingFriendRequestId), { method: "POST", body: JSON.stringify(body) });
        if (typeof res?.newBalance === "number") setWalletCoins(res.newBalance); else refreshWallet();
      } else {
        alert("You can't send a message here yet. Accept the friend request, or open a conversation thread.");
        return;
      }
      setNewMessage(""); setIsGiftModalOpen(false);
      await loadThreadMessages(activeChat); await loadLists({ quiet: true });
    } catch (e) { alert(e.message || "Failed to send"); }
    finally { setSending(false); }
  };

  const currentList = activeTab === "inbox" ? inboxList : activeTab === "requests" ? requestsList : sentList;
  ...
  useEffect(() => {
    ...
    const list = activeTab === "inbox" ? inboxList : activeTab === "requests" ? requestsList : sentList;
    const row = list.find((c) => String(c.conversationId || c.id) === String(rk));
    if (!row) return;
    setActiveChat((prev) => {
      if (!prev || String(prev.rowKey) !== String(rk)) return prev;
      const nu = row.userStatus; const nb = row.isBroadcasting; const bu = row.broadcastUrl;
  ...
  const requestsTabCount = (notif?.breakdown?.receivedRequests ?? 0) + (notif?.breakdown?.friendRequests ?? 0);
  const listHasMore = activeTab === "inbox" ? inboxHasMore : activeTab === "requests" ? requestsHasMore : sentHasMore;
  ...
  const openRow = (row) => {
    ...
    const followId =
      ...
      (typeof row.id === "string" && row.id.startsWith("pending_fr_") ? row.id.replace("pending_fr_", "") : null);
    const outgoingId =
      row.outgoingFriendRequestId ||
      (typeof row.id === "string" && row.id.startsWith("outgoing_fr_") ? row.id.replace("outgoing_fr_", "") : null);
    const clearUnread = (setter) => setter((prev) => prev.map((c) => {
    ...
    setActiveChat({
      rowKey: cid, conversationId: cid,
      otherUser: row.otherUser, otherUserId: row.otherUserId,
      ...
      unreadCount: 0, userStatus: row.userStatus,
      isBroadcasting: row.isBroadcasting, broadcastUrl: row.broadcastUrl,
    });
    if (!isSyntheticConversationId(cid)) void markReadForPeer(row.otherUser?.id || row.otherUserId);
  };

  const showRecipientFollowActions =
    activeTab === "requests" && activeChat && activeChat.isFollowRequest &&
    !activeChat.isOutgoingFriendRequest && messages.length === 0;

  const showComposer =
    activeChat && !showRecipientFollowActions &&
    (activeTab === "inbox" || !activeChat.isFollowRequest || activeChat.isOutgoingFriendRequest ||
      messages.length > 0 || !isSyntheticConversationId(activeChat.conversationId));

  const textInputLocked = showComposer && !activeChat?.isFriend && !canSendTextOnlyNonFriend(messages, currentUserId);
  ...
  const handleUnfriendPeer = async () => {
    ...
    try {
      ...
      await loadLists({ quiet: true, skipNotificationBadge: true });
      scheduleNotificationBadge();
    } catch (e) { alert(e.message || "Could not unfriend"); }
    finally { setThreadActionBusy(false); }
  };

  const handleBlockPeer = async () => {
    if (!peerId || threadActionBusy) return;
    const label = otherProfile?.username || "this user";
    if (!window.confirm(`Block ${label}? They won't be able to message you and pending requests with them will be closed.`)) return;
    setThreadActionBusy(true);
    try {
      await apiRequest(API.FRIENDS.BLOCK_USER(peerId), { method: "POST", body: "{}" });
      setThreadMenuOpen(false); setActiveChat(null);
      await loadLists(); scheduleNotificationBadge();
    } catch (e) { alert(e.message || "Could not block user"); }
    finally { setThreadActionBusy(false); }
  };
        ...
        <div className={clsx('flex', 'min-h-0', 'flex-1', 'w-full', 'flex-col', 'rounded-[48px]', 'md:ring-2', 'md:ring-white/50', 'md:ring-offset-2', 'md:ring-offset-purple-900/90', 'overflow-hidden', 'bg-transparent', 'md:h-[78vh]', 'md:flex-row')}>
          ...
          <div className={`min-h-0 md:w-[60%] w-full h-full flex flex-col p-2 ${activeChat ? "flex" : "hidden md:flex"}`}>
            {activeChat ? (
              <>
                <div className={clsx('border', 'border-white/50', 'rounded-[50px]', 'flex-1', 'flex', 'flex-col', 'overflow-hidden')}>
                  ...
                  {showRecipientFollowActions && (
                    <div className={clsx('px-6', 'py-4', 'flex', 'gap-3', 'justify-center', 'border-b', 'border-white/10')}>
                      <button
                        type="button"
                        ...
                        className={clsx('bg-[#d91e82]', 'text-white', 'text-sm', 'font-bold', 'px-6', 'py-2', 'rounded-full')}
                      >
                        Accept
                      </button>
                      <button
                        ...
                        className={clsx('bg-white/10', 'text-white', 'text-sm', 'font-bold', 'px-6', 'py-2', 'rounded-full')}
                      >
                        Reject
                      </button>
                    </div>
                  ...
                  {textInputLocked && showComposer && (
                    <p className={clsx('px-6', 'pt-2', 'text-[11px]', 'text-amber-200/90', 'font-medium')}>
                      You've sent a message here already — next sends need a gift (tap gift icon).
                    </p>
                  )}
                  {!activeChat.isFriend && showComposer && canSendTextOnlyNonFriend(messages, currentUserId) && walletCoins != null && walletCoins < firstMessageCost && (
                    <p className={clsx('px-6', 'pt-1', 'text-[11px]', 'text-red-300/90')}>
              ...
              <div className={clsx('flex', 'min-h-0', 'flex-1', 'flex-col', 'overflow-hidden', 'rounded-3xl', 'border', 'border-white/50', 'md:rounded-[50px]')}>
                <div className={clsx('flex', 'flex-1', 'items-center', 'justify-center', 'px-4', 'py-12')}>
                  <p className={clsx('text-center', 'text-white/60')}>Select a conversation to start messaging</p>
                </div>
              </div>
            )}
          </div>
