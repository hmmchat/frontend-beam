...
export default function RandomnessModal({
  ...
  const canControlRandomness = !!isLocalHost;
  const pullStrangerLocked = !canControlRandomness || isPullStrangerDisabled;
  const pullStrangerHint = !canControlRandomness
    ? 'Only hosts can pull in a stranger'
    : isRoomFull
      ? 'Room is full (4/4)'
      : isEnablingPullStranger
        ? 'Enabling...'
 ...
 return (
  ...
  <h2 className="relative z-10 text-white text-base md:text-xl font-otomanopee md:font-black tracking-wider">
    Add randomness
      ...
      {canControlRandomness && isBroadcasting ? (
            ...
            /* Traditional single button view — hosts can act; pulled-in strangers see the same options disabled */
            <>
              <button
                onClick={handlePullStranger}
                disabled={pullStrangerLocked}
              ...
              <button
                ...
                disabled={!canControlRandomness}
                className={`relative overflow-hidden w-[90%] mx-auto md:w-full border border-white/90 rounded-[2rem] p-6 flex items-center gap-6 text-left transition-all py-9 md:py-6 ${
                  !canControlRandomness
                    ? 'opacity-50 cursor-not-allowed'
                    : 'hover:scale-[1.02] active:scale-95'
                }`}
              >
                ...
                <div className="relative z-10 flex-1">
                  ...
                  <p className="text-white/90 md:text-[12px] text-[10px] font-outfit md:w-62 w- text-center mt-1.5 mx-auto">
                    {canControlRandomness
                      ? 'Starts streaming this call live on Beam TV. Streamed Chat can gift and join'
                      : 'Only hosts can start Beamcast'}
                  </p>
                </div>
              </button>
            </>
