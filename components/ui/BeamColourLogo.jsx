...
/** Tailwind spacing scale (rem → px at 16px root) for w-* size hints */
const TW_W = {
  "w-20": 80,
  "w-24": 96,
  "w-28": 112,
...
/** Best-effort width hint from className so we never render at native 794px */
function widthHintFromClass(className) {
  const s = String(className);
  // Prefer non-responsive arbitrary width, then md:, then spacing tokens
  const arbitraryBase = s.match(/(?:^|\s)w-\[(\d+(?:\.\d+)?)px\](?:\s|$)/);
  if (arbitraryBase) return Number(arbitraryBase[1]);
  const arbitraryMd = s.match(/md:w-\[(\d+(?:\.\d+)?)px\]/);
  if (arbitraryMd) return Number(arbitraryMd[1]);

  const base = s.match(/(?:^|\s)(w-(?:20|24|28|32|40|44|56|60|64))(?:\s|$)/);
  if (base) return TW_W[base[1]];
  const md = s.match(/md:(w-(?:20|24|28|32|40|44|56|60|64))/);
  if (md) return TW_W[md[1]];
  return null;
}

/**
 * Yellow "beam" wordmark with animated light sweep.
 * Pass a Tailwind width class (e.g. `w-[129px]`, `w-44`, `md:w-60`).
 */
export default function BeamColourLogo({ className = "", alt = "Beam" }) {
  const filterId = useId().replace(/:/g, "");
  const rootRef = useRef(null);
  const hint = widthHintFromClass(className);
  const [scale, setScale] = useState(hint ? hint / NATIVE_W : 0);
  ...
  useLayoutEffect(() => {
    ...
    const update = () => {
      ...
      if (h > 1) {
        const derivedW = h * (NATIVE_W / NATIVE_H);
        el.style.width = `${derivedW}px`;
        setScale(derivedW / NATIVE_W);
        return;
      }
      if (hint) setScale(hint / NATIVE_W);
    };
  ...
  const safeScale = scale > 0 ? scale : hint ? hint / NATIVE_W : 0.2;

  return (
    <div
      ref={rootRef}
      className={clsx("bcl", className)}
      role={decorative ? undefined : "img"}
      aria-label={decorative ? undefined : alt}
      aria-hidden={decorative ? true : undefined}
    >
      <div
        className="bcl__stage"
        style={{ transform: `scale(${safeScale})` }}
        aria-hidden="true"
      >
        <div className="bcl__logo">
          <div className="bcl__beam-clip">
            <div className="bcl__beam-wrap">
              <div className="bcl__beam" />
            </div>
          </div>

          <svg
            className="bcl__outline"
          ...
          >
            <defs>
              ...
              >
                ...
                <feComposite
                  ...
                  result="outline"
                />
                <feOffset in="dilated" dx="15" dy="10" result="shadow-offset" />
                <feMorphology in="dilated" radius="3" result="clip-mask" />
                <feComposite
                  in="shadow-offset"
                  in2="clip-mask"
                  operator="out"
            ...
            <path
              d={LETTER_PATH}
              fill="#F2AD00"
              filter={`url(#colour-outline-${filterId})`}
            />
          </svg>
        </div>
      </div>
