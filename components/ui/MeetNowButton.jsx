...
export default function MeetNowButton({ 
  ...
  return (
    <button
      onClick={onClick}
      className={clsx(
        'group relative z-20 border-[1.89px] border-b-[5.4px] rounded-[20px] flex items-center justify-center gap-4 active:scale-[0.98] transition-all overflow-hidden shadow-2xl hover:scale-[1.01] hover:brightness-110 hover:shadow-purple-500/20',
        isSearching
          ? 'bg-yellow-500 text-black border-black animate-pulse'
          : 'bg-[#0A032D]/30 text-white border-white hover:bg-black/30 ',
        className
      )}
    >
      {!isSearching && (
