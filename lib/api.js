...
export const API = {
    ...
    USERS: {
        ...
        POST_DISCOVERY_USERS: `${USER_SERVICE_URL}/users/discovery`,
    },
    ...
    DISCOVERY: {
        ...
        // TV Broadcasts
        GET_BROADCAST_FEED: (sessionId) => `${DISCOVERY_SERVICE_URL}/discovery/broadcasts/feed?sessionId=${sessionId}`,
...
export const apiRequest = async (url, options = {}) => {
    ...
    if (!response.ok) {
        const error = await response.json().catch(() => ({ message: 'Request failed' }));
        const base = error.message || `HTTP ${response.status}`;
        if (response.status === 429) {
            throw new Error(`${base} — please wait a moment and try again.`);
        }
