...
const DISCOVERY_WS_BASE = process.env.NEXT_PUBLIC_DISCOVERY_WS_URL || 'wss://api.beam.place/v1/notifications/ws';
...
const PAYMENT_SERVICE_URL = `${API_V1}/payments`;  // Matches /payments
const STREAMING_SERVICE_URL = API_V1;    // Matches /streaming
const ADS_SERVICE_URL = `${API_V1}/ads`; // Matches /ads (Gateway strips /ads prefix)
...
export const API = {
    ...
    AUTH: {
        ...
        PHONE_SEND_OTP: `${AUTH_SERVICE_URL}/auth/phone/send-otp`,
        PHONE_VERIFY: `${AUTH_SERVICE_URL}/auth/phone/verify`,
        /** POST body: `{ refreshToken }` — auth-service `POST /auth/logout` */
        LOGOUT: `${AUTH_SERVICE_URL}/auth/logout`,
        /** Authenticated `DELETE /auth/me` — soft-delete / deletion window per backend */
        DELETE_ACCOUNT: `${AUTH_SERVICE_URL}/auth/me`,
        /** Authenticated `GET /auth/me/status` — rejects hard-deleted leftover JWTs */
        GET_STATUS: `${AUTH_SERVICE_URL}/auth/me/status`,
    },
    ...
    USERS: {
        ...
        UPDATE_STATUS: `${USER_SERVICE_URL}/me/status`,
        REPORT_PRESENCE: `${USER_SERVICE_URL}/me/presence`,
        PRESENCE_HEARTBEAT: `${USER_SERVICE_URL}/me/presence/heartbeat`,
        ...
        ADD_PHOTO: `${USER_SERVICE_URL}/me/photos`,
        GET_PHOTOS: `${USER_SERVICE_URL}/me/photos`,
        DELETE_PHOTO: (photoId) => `${USER_SERVICE_URL}/me/photos/${photoId}`,
        UPDATE_INTENT: `${USER_SERVICE_URL}/me/intent`,
        UPDATE_INTERESTS: `${USER_SERVICE_URL}/me/interests`,
        UPDATE_VALUES: `${USER_SERVICE_URL}/me/values`,
        UPDATE_BRANDS: `${USER_SERVICE_URL}/me/brand-preferences`,
        ...
        /** Gateway → user-service: `{ count }` total rows in `users` (all statuses). */
        GET_ACTIVE_MEETINGS: `${USER_SERVICE_URL}/metrics/active-meetings`,
        GET_ANYWHERE_COUNT: `${USER_SERVICE_URL}/metrics/anywhere-count`,
        REPORT: `${USER_SERVICE_URL}/users/report`,
        GET_BADGES: (userId) => `${USER_SERVICE_URL}/users/${userId}/badges`,
        SET_ACTIVE_BADGE: (userId) => `${USER_SERVICE_URL}/users/${userId}/badges/active`,
        /** Moderators only — toggle show-as-moderator vs disguised (show as user). */
        PATCH_MODERATOR_FACE_CARD: `${USER_SERVICE_URL}/me/moderator-face-card`,
    },

    // Files endpoints
    FILES: {
        UPLOAD: `${FILES_SERVICE_URL}/files/upload`,
        IMAGE_PROXY: `${FILES_SERVICE_URL}/files/proxy/image`,
    },
    ...
    FRIENDS: {
        ...
        GET_BLOCKED_USERS: (q = {}) => {
            ...
            if (q.cursor) p.set('cursor', q.cursor);
            const s = p.toString();
            return `${FRIEND_SERVICE_URL}/me/friends/blocked${s ? `?${s}` : ''}`;
        },
        ...
        UNBLOCK_USER: (userId) => `${FRIEND_SERVICE_URL}/me/friends/${userId}/unblock`,
        ACCEPT_FRIEND_REQUEST: (requestId) => `${FRIEND_SERVICE_URL}/me/friends/requests/${requestId}/accept`,
        REJECT_FRIEND_REQUEST: (requestId) => `${FRIEND_SERVICE_URL}/me/friends/requests/${requestId}/reject`,
        CHECK_FRIENDSHIP: (friendId) => `${FRIEND_SERVICE_URL}/me/friends/${friendId}/check`,
    },
    ...
    DISCOVERY: {
        ...
        /** Room assigned after mutual Meet rn (Redis). Prefer this while waiting. */
        MY_ROOM: `${DISCOVERY_SERVICE_URL}/discovery/my-room`,
        /** Active mutual match partner — waiters poll to exit on peer raincheck. */
        MATCH_STATUS: `${DISCOVERY_SERVICE_URL}/discovery/match-status`,
        ...
        AVAILABLE_CITIES: (sessionId, { limit = 3, soloOnly = false, excludeCity } = {}) => {
            const p = new URLSearchParams();
            p.set('sessionId', String(sessionId));
            p.set('limit', String(limit));
            p.set('soloOnly', String(Boolean(soloOnly)));
            if (excludeCity) p.set('excludeCity', String(excludeCity));
            return `${DISCOVERY_SERVICE_URL}/discovery/available-cities?${p.toString()}`;
        },
        UI_CONFIG: `${DISCOVERY_SERVICE_URL}/discovery/ui-config`,
        SESSION_HEARTBEAT: `${DISCOVERY_SERVICE_URL}/discovery/session/heartbeat`,
        SESSION_ENTER: `${DISCOVERY_SERVICE_URL}/discovery/session/enter`,
        SESSION_END: `${DISCOVERY_SERVICE_URL}/discovery/session/end`,
        MEET_RN_WAITING_MESSAGE: `${DISCOVERY_SERVICE_URL}/discovery/meet-rn/waiting-message`,
        // Offline cards
        ...
        ENGAGE_OFFLINE: `${DISCOVERY_SERVICE_URL}/discovery/offline-cards/engage`,
    ...
    /**
     * Squad lobby & invites (gateway: /v1/squad → discovery-service).
     *
     * Quick-invite (backend-owned, not localStorage):
     * - GET QUICK_INVITE_SUGGESTIONS → `{ hasUsedSquadVideo?: boolean, suggestions: { userId, username?, displayPictureUrl? }[] }` (max 3; MRU co-participants from past squad video calls).
     * - POST RECORD_QUICK_INVITE_CALL_PEERS → body `{ peerUserIds: string[] }`; merge into stored MRU list capped at 3 (idempotent per deploy; server may require friendship).
     */
    SQUAD: {
        QUICK_INVITE_SUGGESTIONS: `${API_V1}/squad/me/quick-invite-suggestions`,
        RECORD_QUICK_INVITE_CALL_PEERS: `${API_V1}/squad/me/quick-invite/record-call-peers`,
        INVITE: `${API_V1}/squad/invite`,
        INVITE_EXTERNAL: `${API_V1}/squad/invite-external`,
        JOIN_EXTERNAL: (token) => `${API_V1}/squad/join/${encodeURIComponent(token)}`,
        ACCEPT_INVITATION: (inviteId) => `${API_V1}/squad/invitations/${inviteId}/accept`,
        REJECT_INVITATION: (inviteId) => `${API_V1}/squad/invitations/${inviteId}/reject`,
        RECEIVED_INVITATIONS: `${API_V1}/squad/invitations/received`,
        CANCEL_INVITATION: `${API_V1}/squad/invitations/cancel`,
        PENDING_INVITATIONS_LOBBY: `${API_V1}/squad/invitations/pending/lobby`,
        LOBBY: `${API_V1}/squad/lobby`,
        LOBBY_MEMBERSHIP: `${API_V1}/squad/lobby/membership`,
        REMOVE_MEMBER: `${API_V1}/squad/lobby/remove-member`,
        ENTER_CALL: `${API_V1}/squad/lobby/enter-call`,
        TOGGLE_SOLO: `${API_V1}/squad/toggle-solo`,
    },
    ...
    STREAMING: {
        ...
        GET_CHAT_HISTORY: (roomId, limit = 50, before) => {
            const p = new URLSearchParams();
            if (limit != null) p.set('limit', String(limit));
            if (before) p.set('before', String(before));
            const s = p.toString();
            return `${STREAMING_SERVICE_URL}/streaming/rooms/${roomId}/chat${s ? `?${s}` : ''}`;
        },
        CREATE_ROOM: `${STREAMING_SERVICE_URL}/streaming/rooms`,
        SEND_GIFT: (roomId) => `${STREAMING_SERVICE_URL}/streaming/rooms/${roomId}/gifts`,
        SEND_DARE: (roomId) => `${STREAMING_SERVICE_URL}/streaming/rooms/${roomId}/dares/send`,
        GET_DARE_GIFTS: (roomId) => `${STREAMING_SERVICE_URL}/streaming/rooms/${roomId}/dares/gifts`,
        GET_RANDOM_DARES: (roomId, userId) => `${STREAMING_SERVICE_URL}/streaming/rooms/${roomId}/dares/random?userId=${userId}`,
        GET_SAVED_DARES: (roomId, userId) => `${STREAMING_SERVICE_URL}/streaming/rooms/${roomId}/dares/custom?userId=${userId}`,
        SAVE_CUSTOM_DARE: (roomId) => `${STREAMING_SERVICE_URL}/streaming/rooms/${roomId}/dares/custom/save`,
        DELETE_CUSTOM_DARE: (roomId, dareId, userId) => `${STREAMING_SERVICE_URL}/streaming/rooms/${roomId}/dares/custom/${dareId}?userId=${userId}`,
        ...
        DISABLE_PULL_STRANGER: (roomId) => `${STREAMING_SERVICE_URL}/streaming/rooms/${roomId}/disable-pull-stranger`,
        ...
        GET_LOADING_MEMES: `${STREAMING_SERVICE_URL}/streaming/loading-memes`,
        GET_RANDOM_LOADING_MEME: `${STREAMING_SERVICE_URL}/streaming/loading-memes/random`,
        SEND_OFFLINE_GIFT: `${STREAMING_SERVICE_URL}/streaming/offline-cards/gifts`,
        /** In-call report — pass roomId and/or callSessionId for consecutive-call scoring. */
        REPORT: `${STREAMING_SERVICE_URL}/streaming/users/report`,
        ...
        GET_FAVOURITES_WITH_LIVE_STATUS: (limit = 100) => `${STREAMING_SERVICE_URL}/streaming/favourites?limit=${limit}`,
    ...
    // Discovery notification + presence WebSocket URL (via API gateway)
    DISCOVERY_WS: {
        WS_URL: DISCOVERY_WS_BASE,
    },

    // Wallet endpoints
    WALLET: {
        GET_BALANCE: `${WALLET_SERVICE_URL}/me/balance`,
        GET_GIFTS: `${WALLET_SERVICE_URL}/me/transactions/gifts`,
        PURCHASE_DIAMONDS: `${WALLET_SERVICE_URL}/me/diamonds/purchase`,
    },

    // Mystery Beam Box / Season
    SEASON: {
        GET_MY_SEASON: `${WALLET_SERVICE_URL}/me/season`,
        CLAIM: `${WALLET_SERVICE_URL}/me/season/claim`,
    },

    // Payments / buy-coins
    PAYMENTS: {
        GET_COIN_PACKAGES: `${PAYMENT_SERVICE_URL}/purchase/packages`,
        INITIATE_PURCHASE: `${PAYMENT_SERVICE_URL}/purchase/initiate`,
        VERIFY_PURCHASE: `${PAYMENT_SERVICE_URL}/purchase/verify`,
    },

    // Rewarded ads endpoints
    ADS: {
        GET_REWARD_CONFIG: `${ADS_SERVICE_URL}/ads/reward/config`,
        VERIFY_REWARD: `${ADS_SERVICE_URL}/me/ads/reward/verify`,
        GET_REWARD_HISTORY: (limit = 50, offset = 0) =>
            `${ADS_SERVICE_URL}/me/ads/reward/history?limit=${limit}&offset=${offset}`,
    },

    // Referrals: use /auth/me/* so gateways that proxy /auth but not /referrals still work.
    // Auth-service also exposes GET /referrals/me/overview (when gateway registers /referrals).
    REFERRALS: {
        GET_OVERVIEW: `${AUTH_SERVICE_URL}/auth/me/referral-overview`,
        SHARE_EVENTS: `${AUTH_SERVICE_URL}/auth/me/referral-share-events`,
    },
};
...
export const apiRequest = async (url, options = {}) => {
    ...
    // POST/PUT/PATCH with `Content-Type: application/json` but no body breaks some Fastify stacks
    // (strict JSON parser). Omit Content-Type when there is no body unless the caller set one.
    const hasBody =
