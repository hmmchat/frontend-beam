...
export function formatDiscoveryCityTitle(city) {
  ...
  const c = String(city);
  if (c === 'ANYWHERE_IN_INDIA') return 'Anywhere';
 ...
 */
export function buildDiscoveryCityFaceCardUser({ city, availableCount, faceCardImageUrl }) {
  const title = formatDiscoveryCityTitle(city);
  const count = typeof availableCount === 'number' ? availableCount : 0;
  const people = `${count.toLocaleString()} ${count === 1 ? 'person' : 'people'} meeting now`;
  return {
    id: `location:${city ?? 'anywhere'}`,
    username: title,
    city: people,
    preferredCity: people,
    /** No outlined age chip — same header rhythm as people cards, without a fake age. */
    hideFacecardAge: true,
    intent: `${people} here. Meet rn = this session only; your saved city is unchanged.`,
