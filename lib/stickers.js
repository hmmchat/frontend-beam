...
/** Only stickers with real art and a future expiry belong in the picker. */
export function isDisplayableSticker(badge) {
  const url = typeof badge?.imageUrl === "string" ? badge.imageUrl.trim() : "";
  if (!url) return false;
  if (badge?.expiresAt) {
...
export async function fetchUserStickers(userId) {
  ...
  if (Array.isArray(badgeData) && badgeData.length > 0) {
    return {
      stickerExpiryDays: 7,
      badges: badgeData
        .map((badge) => enrichStickerBadge(badge, catalogMap))
        .filter(isDisplayableSticker),
    };
  }
